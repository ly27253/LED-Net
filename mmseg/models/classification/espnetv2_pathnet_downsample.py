...
class EESPNet0(nn.Module):
    ...
    def __init__(self):
        ...
        self.u1_level2_0 = DownSampler(out_channel_map[1], out_channel_map[2], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement)  # out = 56

        self.u1_level3_0 = DownSampler(out_channel_map[2], out_channel_map[2], k=K[1], r_lim=recept_limit[1], reinf=self.input_reinforcement, Keep_channels_up=False) # out = 28
        ...
        self.u1_level4_0 = DownSampler(out_channel_map[1], out_channel_map[0], k=K[2], r_lim=recept_limit[2], reinf=self.input_reinforcement, Keep_channels_up=False, Keep_scale=True, channels_down=True) #out = 14
        ...
        # self.u1_level3_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     if i < 1:
        #         self.u1_level3_r.append(EESP(out_channel_map[1], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #     else:
        #         self.u1_level3_r.append(EESP(out_channel_map[1], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #
        # self.u1_level2_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     self.u1_level2_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
...
class EESPNet1(nn.Module):
    ...
    def __init__(self):
        ...
        self.u2_level1 = DownSampler(out_channel_map[2], out_channel_map[3], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement)  # out = 56

        self.u2_level2 = DownSampler(out_channel_map[3], out_channel_map[2], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement, Keep_channels_up=False, channels_down=True, Keep_scale=True)  # out = 56

        self.u2_level3_0 = DownSampler(out_channel_map[2], out_channel_map[2], k=K[1], r_lim=recept_limit[1], reinf=self.input_reinforcement, Keep_channels_up=False) # out = 28
        ...
        # self.u2_level3_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     if i < 1:
        #         self.u2_level3_r.append(EESP(out_channel_map[1], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #     else:
        #         self.u2_level3_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #
        # self.u2_level2_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     self.u2_level2_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
...
class EESPNet2(nn.Module):
    ...
    def __init__(self):
        ...
        self.u3_level1 = DownSampler(out_channel_map[3], out_channel_map[4], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement)  # out = 56

        self.u3_level2 = DownSampler(out_channel_map[4], out_channel_map[3], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement, Keep_channels_up=False, channels_down=True, Keep_scale=True)  # out = 56

        self.u3_level3_0 = DownSampler(out_channel_map[3], out_channel_map[3], k=K[1], r_lim=recept_limit[1], reinf=self.input_reinforcement, Keep_channels_up=False, Keep_scale=False)  # out = 28
        ...
        # self.u3_level3_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     if i < 1:
        #         self.u3_level3_r.append(EESP(out_channel_map[1], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #     else:
        #         self.u3_level3_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #
        # self.u3_level2_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     self.u3_level2_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
...
class EESPNet3(nn.Module):
    ...
    def __init__(self):
        ...
        self.u4_level1 = DownSampler(out_channel_map[4], out_channel_map[5], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement)  # out = 56

        self.u4_level2 = DownSampler(out_channel_map[5], out_channel_map[4], k=K[0], r_lim=recept_limit[0], reinf=self.input_reinforcement, Keep_channels_up=False, channels_down=True, Keep_scale=True)  # out = 56

        self.u4_level3_0 = DownSampler(out_channel_map[4], out_channel_map[3], k=K[1], r_lim=recept_limit[1], reinf=self.input_reinforcement, Keep_channels_up=False, channels_down=True, Keep_scale=True)  # out = 28
        ...
        # self.u4_level3_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     if i < 1:
        #         self.u4_level3_r.append(EESP(out_channel_map[1], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #     else:
        #         self.u4_level3_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
        #
        # self.u4_level2_r = nn.ModuleList()
        # for i in range(reps_at_each_level[1]):
        #     self.u4_level2_r.append(EESP(out_channel_map[2], out_channel_map[2], stride=1, k=K[3], r_lim=recept_limit[3]))
